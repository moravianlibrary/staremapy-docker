...
def transformData(header, data):
    ...
    data[geoid_index] = 'http://staremapy.georeferencer.cz/map/%s/georeference' % (geoid)
...
class HTMLWriter:
        ...
        <html lang="cs">
            ...
            <body>
                ...
                <div id="content" class="subpage nonprocessed">
                    <h1>Zbývá zpracovat</h1>
                    <p>Na základě četných žádostí o možnost zpracovat konkrétní mapu jsme jako prozatímní řešení zpřístupnili tuto tabulku. Obsahuje názvy těch dosud nezpracovaných map, ke kterým jsme od jejich vlastníků obdrželi názvové údaje. Vyhledávat v něm můžete například současným stisknutím kláves CTRL+F a zadáním slovního kořene názvu (např. "Klad" pro Kladno, kladenský, kladenského ...). Pokud budete chtít, <a href="/vysledky/zbyva-zpracovat/export.csv">kliknutím na tento odkaz</a> můžete celý soubor stáhnout do svého počítače. Aktualizace tabulky probíhá každých 5 minut.</p>
                    <ul>%s</ul>
        ''' % (''.join(map(lambda x: '<li><a href=#%s>%s</a></li>' % (revMapCollection(x), x) , sorted(collections)))))
    ...
    def writerow(self, row):
        ...
        self.out.write('<td><a target="_blank" href="%s">Zpracovat</a></td>' % (row[1]))
        self.out.write('</tr>')
...
def groupByFunc(data, i):
    ...
    for k, g in groupby(filter(lambda x: len(x) != 0, data), key = lambda x : x[i]):
        if k in groups:
            ...
            groups[k] = list(g)
    return sorted(groups.iteritems(), key = lambda x : len(x[1]))
...
query = "SELECT 'title', 'georeferencer_id', 'collection' FROM 1gpUBes9cpOkH3h-fNyS_4dnuLelIY-bSuWdNfqw WHERE 'status' = ''"
...
if response.status == 200:
    ...
    trebon_index = [x[0] for x in indata].index('Státní oblastní archiv v Třeboni')
    trebon = indata[trebon_index]
    del indata[trebon_index]
    indata.append(trebon)
